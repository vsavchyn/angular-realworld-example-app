...
/**
 * Fulfill Conduit API calls from the same browse-feed fixtures as the mock,
 * so LCP/CLS/TBT stay frontend-owned (not the public API).
 */
...
export async function installLabObservers(page: Page): Promise<void> {
  await page.addInitScript(() => {
    ...
    try {
      ...
      // longtask observer not available (needed for TBT)
    }
  });
}
