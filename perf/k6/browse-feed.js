/**
 * Anonymous browse-feed — the common Conduit session (read-heavy content consumption).
 *
 * Weighted mix: ~80% list traffic (tags + feed + occasional pagination),
 * ~20% article + comments. No register/login/write.
 *
 * Thresholds are tight for the local mock. A self-hosted RealWorld backend
 * would use looser SLOs (e.g. http_req_duration p95 < 500ms).
 *
 * Do not point this script at api.realworld.show.
 */
...
export default function browseFeed() {
  ...
  // ~20% of iterations open the first article (markdown + comments).
  ...
  // ~20% of iterations also paginate the global feed.
  if (Math.random() < 0.2) {
